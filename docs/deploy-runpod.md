...
| **عامل `memory` فقط** | `knowledge` و`media` ينهاران عند الإقلاع (ينقصهما `DocumentContentResolver` و`MediaGenerator`) — دَينٌ مسجَّل، لا خلل نشر |
...
| العامل ينهار عند الإقلاع | إن كان `WORKER=knowledge` أو `media` فهذا **مقصودٌ ومسجَّل**. القيمة العاملة الوحيدة `memory` |
