...
| **عامل `memory` افتراضاً** | `media` وحده ينهار عند الإقلاع (ينقصه `MediaGenerator`) — دَينٌ مسجَّل، لا خلل نشر. و`knowledge` صار موصولاً بالكامل ([§3.100](log/3.100.md)) ويصحّ تشغيله صراحةً؛ الافتراضيّ يبقى `memory` لأنّه المُثبَت حاويّاً |
...
| العامل ينهار عند الإقلاع | إن كان `WORKER=media` فهذا **مقصودٌ ومسجَّل**. وإن كان `knowledge` فهو عطلُ **بيئةٍ** لا نقصُ كود (‏Vault · سرّ MinIO · `PROVIDER_ROUTING`) — الرسالة تسمّيه |
