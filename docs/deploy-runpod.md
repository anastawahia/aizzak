...
| 8 | `[bootstrap] starting workers` → `starting the API` → `starting nginx` | ‏< 1 د | طبقة التطبيق، بهذا الترتيب — **اصطلاحٌ مُبقًى لا ضمانةٌ ملزِمة بعد الآن** (أسفل) |
...
- **ترتيب العمّال ثمّ المُرحّل — مُبقًى اصطلاحاً، ولم يعد ضمانةً:** مجموعات مستهلكي Redis ما تزال تُنشأ عند `$` (ذيل المجرى، بلا تغيير)، وهذا بالذات كان يجعل مجموعةً تُنشأ **بعد** نشر المُرحّل على مجرًى **جديد** لا ترى تلك المدخلات أبداً. لكنّ `outbox-relay` نفسه صار يضمن الأزواج الأربعة كلّها قبل أوّل نشرٍ (`ensure_topology()` في `outbox_relay.py`/`bootstrap.py`، [design/08-local-runbook.md §4](design/08-local-runbook.md)، `stream-topology-plan.md`) — فتسلسل `supervisorctl start worker` قبل `start outbox-relay` هنا **لم يعد يحمل معنى الصحّة**؛ تُرك كما هو تجنّباً لتغيير تسلسل إقلاعٍ حيّ بلا داعٍ، لا لأنّ الصحّة تتوقّف عليه بعد اليوم.
...
6. **`WORKER=memory` هي القيمة الوحيدة العاملة اليوم.**
7. **Vault دائمٌ الآن، لكنّ `$DATA/vault-init/init.json` (المفتاح + التوكن) عاديٌّ غير مُشفَّر على القرص** — انسخه احتياطيّاً (§8.1) قبل أيّ اعتمادٍ حقيقيّ، وراجع مسار الترقية إلى KMS قبل إنتاجٍ فعليّ.
8. **`FIREBASE_PROJECT_ID` الفارغ = تطبيقٌ لا يقلع**، لا مجرّد مصادقةٍ معطّلة.
9. **Pod ‏`healthy` ليس دليلاً على مسار بيانات** — الدليل في `deploy/smoke/`.
