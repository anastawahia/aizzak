...
### 2.3 ⚠️ المنافذ: عنوان الشبكة الداخليّة غير عنوان المضيف

أزيلت خدمات PostgreSQL/PgBouncer/Redis/MinIO الأصلية؛ Compose هو مصدر البنية المحليّة الآن، مع بقاء Ollama خدمةً أصليّة داخل WSL. تحتفظ الخدمات **داخل شبكة Compose** بأسمائها ومنافذها القانونيّة (`pgbouncer:6432`, `minio:9000` …)، بينما تبقى منشورات المضيف المزاحة عقداً ثابتاً للمشغّل وحزام الاختبارات:
...
## 6) المسار (ب): حزام الاختبارات فوق Compose

العناوين التي يقصدها المحضن على المضيف (‏مقيسة 2026‑08‑05):

| المنفذ | الخدمة | الحالة |
|---|---|---|
| 15432 | PostgreSQL 16 الحاوي | ✅ منشور من Compose |
| 16432 | PgBouncer الحاوي | ✅ منشور من Compose |
| 16379 | Redis الحاوي | ✅ منشور من Compose |
| 19000 | MinIO الحاوي | ✅ منشور من Compose |
| 16333 | Qdrant الحاوي | ✅ منشور من Compose |
| 18200 | Vault الحاوي الدائم | ✅ منشور من Compose |
| 11434 | Ollama الأصلي داخل WSL | ✅ مفتوح |
| 8080 | خدمة التضمين الحاويّة | ✅ ينشرها `docker-compose.test.yml` على loopback فقط |

اختبارات التكامل تعمل على قاعدة `aizzak_test` داخل Postgres الحاوي، بدورَي `aizzak_owner` (المُهاجِر) و`app_rw` (المحكوم بـRLS). ملف `.env.test` المحليّ يحمل الاعتمادات الفعلية والعناوين؛ وملف التجاوز لا يُقرأ ولا يزوّد القاعدة تلقائياً.
...
pytest -rs                           # التشغيل المحلي المعتاد؛ الغائب يتخطّى
REQUIRE_LIVE=1 pytest -rs            # الحارس الصارم؛ غياب اعتماد محلي يفشل
pytest -m live_db                    # فوق PG16 الحاوي على 15432
...
> كلّ وسم بنيةٍ محليّة يسبر عنوانه: يتخطّى في التشغيل العادي ويفشل مع `REQUIRE_LIVE=1`. أمّا خطأ اعتماد بعد نجاح المسبار فهو فشلٌ مسموع لا تخطٍّ. وبخاصة MinIO: إذا غاب `.env.test` بعد تدوير الحساب، يستعمل المحضن `_MINIO_SECRET_DEFAULT = "aizzak-test-secret"` ويصل الخادم ثم يفشل `SignatureDoesNotMatch`؛ وحين يكون اسما السر مضبوطين معاً يرفض حارس D‑1 اختلافهما قبل I/O.
...
| الأحداث لا تصل العمّال | حالة `outbox-relay`؛ `published_at` في `platform.outbox`؛ طول المجرى؛ **ووجود مجموعة المستهلك وتأخّرها** (`XINFO GROUPS stream.<وحدة>`). **ترتيب الإقلاع لم يعد سبباً محتملاً** (§4) |
