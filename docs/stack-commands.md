...
>
> **آخر مطابقةٍ مع الواقع: 2026‑08‑04.**
...
| 29 | `pytest` | البوّابة 5 — الحزمة كاملةً؛ ما لا تجد خدمته يتخطّى نظيفاً |
| 30 | `pytest -rs` | المِثل، مع طباعة **اسم وسبب** كلّ تخطٍّ — الفرق بين تخطٍّ سليمٍ ومقنّع |
| 31 | `pytest -m live_db` | مسبارٌ حيٌّ واحد فوق الخدمة الأصليّة (وكذا `live_qdrant`, `live_minio` …) |
...
⚠️ **المنافذ مُزاحةٌ على المضيف وحده.** المكدّس يعمل **بجانب** الخدمات الأصليّة على هذا المضيف لا بدلاً منها، فداخل شبكة Compose يحتفظ كلّ شيءٍ باسمه ومنفذه القانونيّ (`pgbouncer:6432`, `minio:9000` …) بينما النشر على المضيف مُزاح: ‏15432 (‏PG) · 16432 (‏PgBouncer) · 16379 (‏Redis) · 19000/19001 (‏MinIO) · 16333 (‏Qdrant) · 18200 (‏Vault). خدمة `embedding` **غير منشورةٍ إطلاقاً** (`expose` لا `ports`) — نصّ المستأجِر يعبر الشبكة الداخليّة إليها، الحدّ الائتمانيّ نفسه الذي لـPostgres.
...
الحزمة كاملةً. كلّ وسم `live_*` **يسبر المنفذ ويتخطّى تخطّياً حقيقيّاً** عند غيابه — لا تمريراً مقنّعاً؛ فـ`SKIPPED: no live embedding service reachable` نتيجةٌ صحيحة لا عطل (خدمة التضمين **حاويّةٌ فقط**، فمنفذها 8080 مغلقٌ على المضيف — إلّا تحت الصيغة رقم **35** أدناه، وهي وحدها ما ينشره).
...
مسبارٌ واحد فوق الخدمات الأصليّة (وكذا `live_qdrant` · `live_minio` · `live_vault` · `live_ollama` …). قاعدة الاختبار `aizzak_test` بدورَي `aizzak_owner` (المُهاجِر) و`app_rw` (المحكوم بـRLS)، والمنحُ خطوةُ `conftest`. كلّ عنوانٍ قابلٌ للتجاوز: `TEST_DATABASE_URL` · `TEST_REDIS_URL` · `TEST_MINIO_ENDPOINT` · `TEST_QDRANT_URL` · `TEST_VAULT_ADDR` · `TEST_OLLAMA_BASE_URL` · `TEST_EMBEDDING_URL`.
...
⚠️ **ولا يهاجر أيّ اختبارٍ بهذا الأمر.** حزمة `tests/integration/` لا تزال تقصد العنقود الأصليّ على `127.0.0.1:5432`؛ الحاويّة على `15432`، والكتلتان منفصلتان تماماً. توجيه الحزمة إلى الثانية تغييرٌ في `conftest.py` لم يُنفَّذ بعد.
