...
class RoutedAnswer:
    ...
    summary_target_name: str | None         # اسمُ الملفّ الذي صُفَّ بناؤه · `None` ⇒ لا بناءَ أو لا اسم
    summary_blocked: SummaryBlocked | None  # سببُ رفضِ البناء · `None` ⇒ لم يُرفَض
    stored_summary_text: str | None         # ملخّصٌ مبنيٌّ سلفاً يُعاد في الدور · `None` ⇒ لا مخزون
    best_dense_score: float | None          # أعلى درجةٍ خامٍ للساق الكثيفة · `None` ⇒ لا بحثَ أو لا نتيجة
    best_bm25_score: float | None           # أعلى درجةٍ خامٍ لساق BM25 · `None` ⇒ كذلك
...
# و`summary_target_name` (خطّة السيناريوهات §٤، ب‑٧أ، الفجوة ف‑٢) اسمُ الملفّ الذي **صُفَّ
# بناؤه**، وهو ما يجعل `summary_job_id` مقروءاً لمن سأل. وقاعدةُ «لا تردّد اسماً لم تحلَّه»
# لا تُنقَض هنا: هي قاعدةُ **الوكيل** — من يردّد صياغةَ السؤال كاسمِ ملفٍّ يزعم حلّاً لم يجرِه
# — ولا محلَّ لها في الوحدة، فالوحدةُ **هي** الحالّ: `resolve_file` جرى واختار مستنداً بعينه
# ورفض سواه، وهذا الحقلُ يحمل `ResolvedFile.file_name` الخاصَّ بذلك الاختيار. وأثرُه أنّ
# مطابقةً ضبابيّةً عند ٠٫٧٨ فوق عتبة ٠٫٧٥ تصير **مرئيّةً في الإيصال نفسِه** بدل أن تُكتشف
# بعد دقائق حين يصل ملخّصُ المستند الخطأ — أو، على مسار التثبيت (س‑٢١)، ألّا تُكتشف أبداً.
# و`None` صادقةٌ وتبقى ممكنة: مستندٌ لم يعد ملفُّه مقروءاً لا اسمَ له يُعطى، فيعود المستدعي
# إلى صياغته غير المسمّاة لا إلى فراغ — وهي `None` أيضاً على كلّ مخرجٍ لم يصفّ بناءً.
# و`summary_blocked` (خطّة السيناريوهات §٥، ب‑٤ب، الفجوة ف‑٧) **سببُ** رفضِ بناءٍ طُلب: المخرجُ
# الرابع للمسار. وُجد لأنّ المستدعي **لا يقدر** على استنتاجه — `RequestSummary` ترفض من موضعين
# وكلاهما `ConflictError` بالرمز `common.conflict` عينِه، فمن يلتقط الاستثناء يعلم أنّ تعارضاً
# وقع ولا يعلم أيَّهما، ثمّ يلزمه أن يكتب جملةً واحدةً عن حقيقتين متضادّتين: ملخّصٌ **قادم**،
# ومستندٌ لا نصَّ فيه ولن يكون. والجملةُ المحايدةُ الصادقةُ في الحالتين (ب‑٤أ) كانت الجوابَ
# الأمين عن ذلك، وهذا الحقلُ هو ما يُقاعدها. وهو **سببٌ لا جملة**: الوحدةُ تصنّف لأنّها وحدَها
# تقدر، ولا تصوغ لأنّ قاعدة س‑18 قائمة — الصوتُ ولغتُه وصياغتُه للمستدعي، وهي القسمةُ نفسُها
# التي يرسمها `clarification_options` حقلاً واحداً قبله. ولا يجتمع مع `summary_job_id` أبداً:
# أحدُهما قبولٌ والآخر رفض. أمّا `summary_target_name` **فيبقى** مع الرفض عمداً — الرفضُ عن
# مستندٍ بعينه، وتسميتُه هي ما يجعل «تعذّر البدء» جملةً يُبنى عليها فعل. وبلا قيمةٍ افتراضيّة
#
# و`stored_summary_text` (خطّة السيناريوهات §٦، ب‑٨، الفجوة ف‑٣) نصُّ ملخّصٍ **مبنيٍّ سلفاً**
# يُعاد في الدور الذي سُئل فيه: المخرجُ الخامس، والوحيدُ الذي يحمل **جواباً** لا خبراً عن بناء.
# وُجد لأنّ `POST` يبني دائماً — وهو صوابٌ في الـREST حيث للقراءة فعلٌ آخر اسمه `GET` — **والدردشة
# لا تملك `GET`**: «لخّص هذا الملفّ» جملةٌ واحدة تعني الأمرين، فمن سأل عن ملخّصٍ بُني بالأمس كان
# يدفع map‑reduce كاملاً وينتظر دقائق لنصٍّ محفوظ. والفحصُ القائم لا يمنع ذلك ولم يُوضع له: يبحث
# عن وظيفةٍ **نشطة**، والمنتهيةُ نقيضُ النشطة. ⚠️ وعقدُ `RequestSummary` **لم يُمَسّ**: القراءةُ في
# **الموجّه** وحده، الطبقةِ التي تعرف أنّ نداءها جاء من محادثة لا من HTTP.
# وهو **نصٌّ لا مُعرِّف** لأنّه لا وظيفةَ هنا أصلاً: بقيّةُ المخارج تُخبر عن بناءٍ قُبل أو رُفض أو لم
# يُستهدَف، وهذا يُخبر عن **جوابٍ**، ولا حاملَ أميناً للجواب إلّا الجواب. وهو الحقلُ الوحيد على هذا
# السجلّ الذي يصل **مُصاغاً**، والاستثناءُ محتجٌّ له لا مفترَض: س‑18 تُعطي الصوتَ للمستدعي وما زال
# له — فليست هذه صياغةَ الوحدة لشيء، بل الملخّصُ نفسُه مؤطَّراً بالإطار عينِه الذي يؤطّره به العاملُ
# حين يسلّمه إلى خيط (`delivered_summary_text`: تنبيهُ الاقتطاع، وترويسةُ اسم الملفّ). وتسليمُ
# `Summary` بدلاً منه كان ينقل التأطير إلى المستدعي فيصير للصفّ المخزَّن شكلان بحسب من سلّمه —
# وذاك هو العطبُ لا المبدأ. و`None` على كلّ مخرجٍ آخر، ومنه المخرجُ الذي يصفّ بناءً: لا مخزونَ،
# ولذلك صُفَّ البناء. ولا يجتمع مع `summary_job_id` ولا مع `summary_blocked`: طلبٌ يُجاب من
# المخزون، أو يُقبل، أو يُرفض. وبلا قيمةٍ افتراضيّة كسابقَيه: مسارٌ يقرأ مخزوناً ثمّ ينسى حمله يعيد
# بناءَ ما أثبتت القراءةُ أنّه موجود، صامتاً، وتلك هي ف‑٣ مُستعادة.
# لسبب `summary_target_name` حرفاً بحرف: رفضٌ نسي أن يُبلّغ عن نفسه يرث `None` فلا يُميَّز عن
# جوابٍ عاديّ، وتلك هي ف‑٧ عائدةً.
#
# و`best_dense_score`/`best_bm25_score` (خطّة السيناريوهات §٨، ب‑١١، الفجوة ف‑٦) الحقلان الثامن
# والتاسع، والوحيدان اللذان يصفان **البحث** لا الجواب: أعلى `VectorHit.score` **خام** أعادته كلُّ
# ساقٍ من ساقَي البحث، على كلّ ما أعادته — قبل التصفية بالصلة، وقبل التنويع، وقبل القصّ إلى `k`.
# وهما إشارتا `RetrievalResult` نفسُهما تُمرَّران بلا تغيير.
# ⚠️ **يعبران ليُقاسا لا ليَحكُما**، وهذا هو تمامُ البند: ق‑٢ تُبقي العتبةَ العدديّة خارج الوكيل،
# وت‑١ هي سببُ جواز بقائها خارجَه — فالعتباتُ موجودةٌ هنا، **مضبوطةٌ ومطبَّقةٌ** طبقةً أدنى. والناقصُ
# لم يكن مِقبضاً بل **رؤية**: ما من سجلٍّ يقرن **مآلَ** الدور بالثقة التي كان الاسترجاعُ يحملها فيه،
# فتوزيعُ الدرجات على الأدوار المنتهية باعتذارٍ مقابلَ المنتهية بجوابٍ لم يكن يُعرَف إلّا تخميناً.
# وبعبورهما تُجمَع المادّةُ **من الإنتاج مجّاناً**، على متنٍ حقيقيٍّ لا على مجموعةِ خمسةَ عشرَ سؤالاً،
# وهي المادّةُ التي تُشتقّ منها أيُّ **إعادةِ** معايرةٍ لاحقة.
# و`None` إشارةٌ غائبةٌ بصدقٍ لا صفر: تلك الساقُ لم تُعِد شيئاً. وهما `None` على كلّ مخرجٍ من مخارج
# مسار التلخيص، فلا استعلامَ جرى فيها أصلاً — إيصالٌ ورفضٌ وملخّصٌ مخزونٌ وسؤالُ توضيحٍ أخبارٌ عن
# مستندات لا نتائجُ بحث، و`0.0` فيها يُقحم صفراً مُختلَقاً في التوزيع الذي وُجد الحقلان لإظهاره.
# وبلا قيمةٍ افتراضيّة كسابقَيه: مسارٌ بحث ثمّ نسي أن يُبلّغ ثقتَه لا يُميَّز عن مسارٍ لم يبحث، وذلك
# هو العمى الذي تسمّيه ف‑٦.
