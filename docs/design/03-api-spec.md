...
## 5) CORS — قرارٌ لا تفصيل (P1‑7)

**v1 لا تشحن `Access-Control-Allow-Origin` قصداً، وهذا قرارٌ لا فجوة.** الافتراض الصريح: الواجهة الأماميّة **تُخدَم من الأصل نفسه** الذي يخدم هذا الـAPI — خلف حافّة nginx الواحدة نفسها (`deploy/nginx/app-locations.conf`، `location /`)، بنفس المخطّط والمضيف والمنفذ. طلبٌ من نفس الأصل **لا يحتاج CORS إطلاقاً** — المتصفّح لا يفرض Same-Origin Policy على طلبٍ لا يعبر أصلاً أصلاً.

**لذلك، عمداً:**
- **لا `CORSMiddleware`** في `src/app` — لا في الكود ولا "احتياطاً".
- **لا كتلة CORS في nginx** (لا `add_header Access-Control-*`، لا معالجة `OPTIONS` مخصّصة).
- **لا ذكر لأصولٍ مسموحة** في أيّ إعداد — لا قائمة، ولا `*`.

إضافة أيٍّ من هذه "احتياطاً" هي بالضبط السطح الذي يُغلقه هذا القرار: وسيطٌ CORS مُفعَّلٌ بلا استخدامٍ فعليّ هو سطح هجومٍ مجّانيّ (نفس منطق `transit/rewrap` في `deploy/vault/app-policy.hcl` — امتيازٌ لا يُمارَس امتيازٌ مجّانيّ).

**إن تغيّر هذا الافتراض يوماً** (واجهةٌ أماميّةٌ من أصلٍ مختلف — نطاقٌ منفصل، أو تطبيقٌ على منفذٍ مختلف أثناء التطوير، أو SPA يُخدَم من CDN منفصل): CORS يصير عندها **تغييراً مطلوباً صريحاً**، لا إضافةً استباقيّة، ويُنفَّذ بـ:
- قائمة أصولٍ **صريحة** (لا `*`) في `CORSMiddleware` أو في nginx — الأصول التي تُخدَّم منها الواجهة فعلاً، لا أكثر.
- **لا `*` مع `Access-Control-Allow-Credentials: true` معاً** — هذا الطلب يحمل `Authorization: Bearer` أصلاً لا كوكيز، فلا حاجة لـ`credentials: true` في الحالة النمطيّة؛ لو استُخدمت الكوكيز يوماً (لا تُستخدم اليوم) فالقاعدة تصير إلزاميّة لا اختياريّة.
- قرارٌ يُتّخذ ويُوثَّق **عند تلك اللحظة**، لا قبلها — هذا القسم نفسه يُحدَّث حينها ليعكس القرار الجديد بدل حذفه.
