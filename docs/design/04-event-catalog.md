...
> **`FileDeleted` بلا نجمة — ومع ذلك حذفُ الملفّ يُفرِغ فهرسه.** بقي الحدث بلا مُستقبِل في `knowledge` فترةً، وكانت تلك ثغرةً لا تصميماً: الملفّ يختفي من الواجهة ويبقى مستنده `indexed` وقطعه ونقاطه قابلة للبحث، فيستشهد الوكيل بملفّ محذوف — وهي نفسها الطريقة التي وُلد بها عطب «الملفّ مفهرَس مرّتين» (احذف ثمّ ارفع، فتتعايش نسختان). ما يَنقل الحذف اليوم **نداءٌ لا مظروف**: `framework/di/file_deletion.py::DeleteFileService` يُشغّل الوسم ثمّ تطهير المعرفة تِباعاً، على غرار تسلسل حذف المساحة تماماً. اختيار التسلسل المتزامن على ترقية الحدث إلى `stream.files` مُعلَّلٌ في وثيقة تلك الوحدة: عامل المعرفة لم يعد مشترَكاً في `stream.files` أصلاً (§4 أعلاه)، والنشر غير المتزامن يترك نافذةً — غير محدودة عند تعطّل العامل — يبقى فيها الملفّ المحذوف قابلاً للاستشهاد، وهي عين العطب المُصلَح.
