...
- مثال: `rag_agent → data_analysis_agent → image_agent → video_agent → file_editing_agent` — خطوات خفيفة متزامنة، والثقيلة تُحال إلى Streams (D‑04). للـWorkflow **محادثته الخاصة** (D‑12).
...
> **تصحيح 4.7‑أ (تناقض مُسجَّل، حُسم):** كانت النسخة السابقة ترسم `LlmResult(prompt_tokens, completion_tokens)` **بعد** حلقة البثّ — و`LLMProvider.stream` لا يسلّم `LlmResult` أبداً، بل `AsyncIterator[LlmChunk]` فقط. صار المخطّط مطابقاً للمنفذ: العدّادات (و`tool_calls` مُجمَّعة) تصل على **الدفعة الأخيرة** من البثّ، وهي الحقول التي أضافها تعديل 4.7‑أ لـ`LlmChunk` (`02 §1.1`). وحين تكون العدّادات `None` (لم يُبلّغ المزوّد) **يُقدِّر المُنسِّق ويَسِم الشحنة بأنها تقديرية** — لا يخلط رقماً مقيساً بآخر مُقدَّراً في سجلّ فوترة بلا أثر يميّزهما.
...
sequenceDiagram
    ...
    alt رفض — allowed=false
        ...
        loop بثّ تدريجي
            ...
            LLM-->>Orch: LlmChunk(delta)
            Orch-->>Client: event token (SSE أو WebSocket)
        end
        LLM-->>Orch: LlmChunk الأخيرة (finish_reason · tool_calls · prompt_tokens · completion_tokens)
