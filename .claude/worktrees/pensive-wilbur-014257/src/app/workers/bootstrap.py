...
class DocumentContentResolver(Protocol):
    ...
    ``build_knowledge_worker_from_env``'s docstring: ``StorageProvider``'s
    MinIO adapter exists but needs an async secret read
    ``CompositionRoot.from_env`` itself already defers to the Phase-6
    lifespan, and no content-extractor DISPATCH composition exists yet
    across the five 3.k1 parser adapters either). Tests inject a fake.
    """
...
def build_knowledge_worker_from_env() -> tuple[
    StreamConsumer, list[Subscription], list[Disposable]
]:
    """Build everything about the knowledge worker that genuinely IS real
    from env (2.10 adds the ``IndexDocument`` pipeline to this list), then
    RAISE naming exactly what remains missing (module docstring's
    "Honest-failure rule").

    Real today: ``documents`` (``SqlDocumentRepository``), ``outbox``
    (``SqlEventOutbox``), ``tenant_session`` (both the ``UnitOfWork`` and
    the RLS session provider), the Redis client/consumer identity, AND --
    since 2.10 -- the full ``IndexDocument`` pipeline itself: ``vectors`` (a
    real ``QdrantVectorStore``) and ``embeddings`` (the
    ``ExternalEmbeddingProvider`` adapter over the central embedding
    service). NOT real: ``DocumentContentResolver``'s file-fetch +
    content-extraction pipeline (``StorageProvider``'s MinIO adapter exists
    but needs an ASYNC secret read that ``CompositionRoot.from_env`` itself
    already defers to the Phase-6 lifespan -- this function has the
    identical constraint, being an equally synchronous factory -- and no
    content-extractor DISPATCH composition exists yet across the five 3.k1
    parser adapters). Raising here, rather than wiring a fake in its place,
    is the design brief's explicit instruction; ``build_knowledge_worker``
    ...
    raise AppError(
        ...
        "(the file-fetch + content-extraction pipeline is not wired from "
        "env today -- a separately tracked debt item, not closed by 2.10); "
        "documents/outbox/ledger/tenant_session/the Redis client/the "
        "IndexDocument pipeline (embeddings + vectors) above ARE real -- "
        "only DocumentContentResolver is blocked. build_knowledge_worker "
        "itself is unaffected by this gap; see its own docstring.",
