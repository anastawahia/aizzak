...
root then reads the Vault secret, constructs the 2.4 ``MinioStorage`` adapter,
and binds it into the handle. The app's Phase-6 startup lifespan calls
``connect_storage`` before the process serves traffic (``api.main``'s
...
# `connect_storage`'s docstring explains the split.
...
def _build_vault(settings: Settings) -> tuple[hvac.Client, SecretsProvider, VaultHealth]:
    """The three Vault objects, built as one unit -- a helper so ``from_env``
    stays under its statement ceiling, the ``_build_embedding``/
    ``_build_metrics_source`` precedent.

    ONE client, shared three ways on purpose:

    * ``vault_client`` is exposed raw for the shutdown-``close()`` precedent
      every other driver on this root follows (``disposables()``).
    * ``VaultSecrets`` gets a ``relogin`` closure (7.3) because AppRole tokens
      ...
      the app loses Vault an hour after boot while still answering
...
class CompositionRoot:
    ...
    def from_env(cls) -> CompositionRoot:
        ...
        # 2.6 + 7.3 + ن-10 -- the three Vault objects, built together (helper
        # above: `_build_vault`) so `from_env` stays under its statement
        # ceiling, the `_build_embedding`/`_build_metrics_source` precedent.
        vault_client, secrets, vault_health = _build_vault(settings)
    ...
    async def connect_storage(self) -> None:
        ...
        traffic. Failure policy is fail-fast, the ``FIREBASE_PROJECT_ID``/D5
        boot precedent: Vault unreachable or a malformed secret raises out of
        startup and the process refuses to boot, rather than booting a replica
        whose every file operation would 500. Idempotent: a lifespan re-entered
        in tests re-runs its hooks, and re-reading the secret to rebuild an
        adapter that is already live would be pure churn.
        """
        if self.storage.is_bound:
            return
        secret = await self.secrets.get_secret(_MINIO_SECRET_PATH)
        ...
        client = create_minio_client(
            self.settings.minio, access_key=access_key, secret_key=secret_key
        )
        ...
        signing_client = create_minio_signing_client(
            self.settings.minio, client, access_key=access_key, secret_key=secret_key
        )
        self.storage.bind(MinioStorage(client, self.settings.minio.bucket, signing_client))
        _logger.info(
            "composition_root.storage_connected",
            extra={
                "endpoint": self.settings.minio.endpoint,
                "signing_endpoint": self.settings.minio.signing_endpoint,
